...
/// ChatPanelView
/// 
/// Implementación robusta basada en el Informe Técnico para resolver
/// fallos de renderizado en Web (Pantalla Blanca).
/// 
/// Arquitectura:
/// - LayoutBuilder: Previene colapso de constraints.
/// - ClipRRect: Mandatorio para contener el BackdropFilter en Web.
/// - Stack: Desacopla la capa de Filtro (Fondo) de la capa de Contenido (UI).
...
class _ChatPanelViewState extends ConsumerState<ChatPanelView> {
  ...
  Widget build(BuildContext context) {
    ...
    // Lógica de Red (Comunicación con HTML)
    ...
    // MOUSE REGION para captura de movimiento dentro del chat
    return MouseRegion(
      ...
      // PILLAR 2 DEL INFORME: Gestión Explícita de Restricciones
      child: LayoutBuilder(
        builder: (context, constraints) {
          return Container(
            width: double.infinity,
            height: double.infinity,
            clipBehavior: Clip.none, // Dejamos que el ClipRRect interno maneje el recorte
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(28),
              border: Border.all(color: borderColor, width: 1.5),
              boxShadow: panelShadows,
              // IMPORTANTE: No ponemos color de fondo aquí para dejar trabajar al filtro
            ),
            
            // PILLAR 1 DEL INFORME: Recorte Estricto
            // ClipRRect es obligatorio como padre directo del BackdropFilter
            child: ClipRRect(
              borderRadius: BorderRadius.circular(28),
              
              // PILLAR 3 DEL INFORME: Aislamiento de Capas
              child: Stack(
                fit: StackFit.expand, 
                children: [
                  // --- CAPA 0: FONDO Y EFECTO DE VIDRIO ---
                  Positioned.fill(
                    child: BackdropFilter(
                      filter: ImageFilter.blur(sigmaX: 10, sigmaY: 10),
                      child: Container(
                        decoration: BoxDecoration(
                          gradient: bgGradient, // Gradiente con opacidad
                        ),
                      ),
                    ),
                  ),

                  // --- CAPA 1: CONTENIDO UI (Mensajes, Inputs) ---
                  // Esta capa vive ENCIMA del filtro, no dentro de él.
                  Column(
                    children: [
                      // HEADER
                      SizedBox(
                      ...
                      Expanded(
                        child: Container(
                          // Capa sutil para mejorar legibilidad sobre el vidrio
                      ...
                      Container(
                        ...
                        // Gradiente inferior para mejorar contraste del input
