...
class _ChatPanelViewState extends ConsumerState<ChatPanelView> with WidgetsBindingObserver { 
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // IMPORTANTE: NO llamamos a setOnline() aquí ciegamente.
      // Solo si por alguna razón ya nacimos abiertos (ej: deep link)
      if (ref.read(chatOpenProvider)) {
         _presenceManager?.setOnline();
      }
    });
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    // Si cierran la pestaña o minimizan la app, cortamos señal.
    if (state == AppLifecycleState.detached || state == AppLifecycleState.paused) {
      _presenceManager?.setOffline();
    } else if (state == AppLifecycleState.resumed) {
      // Si vuelven y el chat estaba abierto, reconectamos.
      if (ref.read(chatOpenProvider)) {
        _presenceManager?.setOnline();
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    // --- NUEVO: ESCUCHAR APERTURA/CIERRE ---
    // Aquí es donde ocurre la magia real de encender/apagar
    ref.listen(chatOpenProvider, (previous, isOpen) {
      if (isOpen) {
        _presenceManager?.setOnline(); // 💡 Luz prendida
      } else {
        _presenceManager?.setOffline(); // 🌑 Luz apagada
      }
    });
    // ----------------------------------------
    ...
    // --- LOGICA DE RED (Alertas visuales) ---
                          ...
                          children: [
                            ...
                            Positioned(
                              ...
                              child: Row(mainAxisSize: MainAxisSize.min, children: [
                                  ...
                                  _ControlButton(icon: Icons.close_rounded, color: iconColor, onTap: () {
                                    // El cierre ya lo maneja el listener de chatOpenProvider, 
                                    // pero por redundancia no hace daño llamar a offline aquí.
                                    _presenceManager?.setOffline();
